...
def test_complete_phishing_analysis():
    ...
    assert "urgent" in results["urgency"]
    assert "verify your account" in results["credentials"]
    assert "suspicious activity" in results["threats"]
    assert "credit card" in results["payment"]

    
    assert len(url_results["suspicious_urls"]) == 2
